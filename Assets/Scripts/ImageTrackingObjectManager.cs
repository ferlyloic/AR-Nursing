...
public class ImageTrackingObjectManager : MonoBehaviour
{
    ...
    private static float currenTime;
    ...
    {
        //Debug.Log("Start method ImageManagerOnTrackedImagesChanged");
        // added, spawn prefab
        foreach (ARTrackedImage image in obj.added)
        {
            if (image.referenceImage.guid == s_FirstImageGUID)
            {
                Debug.Log("First Marker detected");
                m_SpawnedOnePrefab = Instantiate(m_TwoPrefab, image.transform.position, image.transform.rotation);
                m_SpawnedOnePrefab.name = Patient.getAll()[0].id;
                Debug.Log("one Prefab loaded");
                m_OneNumberManager = m_SpawnedOnePrefab.GetComponent<NumberManager>();
            }
            else if (image.referenceImage.guid == s_SecondImageGUID)
            {
                Debug.Log("Second Marker detected");
                m_SpawnedTwoPrefab = Instantiate(m_TwoPrefab, image.transform.position, image.transform.rotation);
                m_SpawnedTwoPrefab.name = Patient.getAll()[1].id;
                Debug.Log("patient Prefab loaded: " + m_SpawnedTwoPrefab);
        ...
        foreach(ARTrackedImage image in obj.updated)
        {
            ...
            if (image.trackingState == TrackingState.Tracking)
            {
                if (image.referenceImage.guid == s_FirstImageGUID)
                {
                    //if (ARNursingGame.currentTime != currenTime)
                    //{
                    //    Debug.Log("time to update");
                    //    currenTime = ARNursingGame.currentTime;
                    //    Destroy(m_SpawnedOnePrefab);
                    //    m_SpawnedOnePrefab = Instantiate(m_TwoPrefab, image.transform.position, image.transform.rotation);
                    //    Patient p = Patient.getAll()[0];
                    //    Debug.Log(p);
                    //    m_SpawnedOnePrefab.name = p.id;
                    //    Debug.Log("one Prefab reloaded");
                    //}
